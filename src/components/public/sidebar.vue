  ...
  <div class="sidebar">
    ...
    <ul class="miansub">
      <li
        ...
        @click="getNavigation(index, item.url)"
      >
        <i :class="item.icon"></i>{{ item.title }}
      </li>
    </ul>
    <div class="help"><i class="el-icon-warning-outline"></i>{{ $t("sidebar.help") }}</div>
  </div>
</template>
...
export default {
  data() {
    return {
      isIndex: window.localStorage.getItem("isIndex") ? window.localStorage.getItem("isIndex") : 1,
    };
  },
  computed:{
    navigations() {
  ...
  methods: {
    getNavigation(index, url) {
      this.isIndex = index;
      window.localStorage.setItem("isIndex", index); //选中
      //this.$router.push({ path: url }); //跳转地址
